...
internal class YoloOutputParser
{
    ...
    private BoundingBoxDimensions ExtractBoundingBoxDimensions(float[] modelOutput, int x, int y, int channel) => new()
    {
    ...
    private CellDimensions MapBoundingBoxToCell(int x, int y, int box, BoundingBoxDimensions boxDimensions) => new()
    {
    ...
    public static IList<YoloBoundingBox> FilterBoundingBoxes(IList<YoloBoundingBox> boxes, int limit, float threshold)
    {
        ...
        var sortedBoxes = boxes.Select((b, i) => new
                {
                    Box = b,
                    Index = i,
                }
